function res = boundary_condition(Qh, Q, c, dt, dx, area, gamma, res, rho_end, u_end, p_end);
 ...
 % compute R1, R2, and R3
    ...
    R2 = - (dt/dx) * ((u_jmax + c(end)) * (Q(2, end) - Q(2, end-1) + (Q(2,end) * c(end)) * (Q(2, end) - Q(2, end-1))) - Q(1,end) * Q(2,end) * c(end)^2 / area(end) * (area(end) - area(end-1)));
    R3 = - (dt/dx) * ((u_jmax - c(end)) * (Q(2, end) - Q(2, end-1) - (Q(2,end) * c(end)) * (Q(2, end) - Q(2, end-1))) - Q(1,end) * Q(2,end) * c(end)^2 / area(end) * (area(end) - area(end-1)));
